...
class BuySellTest extends TestCase
{
    ...
    {
        $user = User::factory()->hasProfile()->create(['email_verified_at' => now(),]);
    ...
    public function test_mypage_buy()
    {
        $user = User::factory()->hasProfile()->create(['email_verified_at' => now(),]);
        $merchandise = Merchandise::factory()
            ->hasCategories(1)
    ...
    public function test_address()
    {
        ...
        $response = $this->actingAs($user)->get("/purchase/address/{$item_id}")->assertStatus(200);
    ...
    public function test_address_buy()
    {
        $user = User::factory()->hasProfile()->create(['email_verified_at' => now(),]);
        $merchandise = Merchandise::factory()
            ->hasCategories(1)
        ...
        $response = $this->assertDatabaseHas('profiles', [
                        'post_code' => '123-4567',
                        'address' => '京都',
                        'building' => 'aaa',
        ]);
    ...
    {
        $user = User::factory()->hasProfile()->create(['email_verified_at' => now(),]);
        Category::factory()->create(['id' => 20]);
        $categoryId = 20;

        $response = $this->actingAs($user)->get('/sell')->assertStatus(200);
